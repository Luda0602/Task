...

{
  ...
  // тип результату
  type ShakerSortResult = {
    sorted: number[];
    swapCount: number;
  };

  // сортування змішуванням
  function shakerSort(array: number[]): ShakerSortResult {
    const data: number[] = array.slice();
  ...
  // масив
  const numbers: number[] = createNumbersArray(30, 1, 100);

  // результат
  const result: ShakerSortResult = shakerSort(numbers);

...

{
  ...
  function sortByNameLength(list) {
    for (let step = 1; step < list.length; step++) {
      let currentItem = list[step];
      let previousPos = step - 1;
  ...
  // Бінарний пошук
  function searchByLength(list, valueLength) {
    let leftSide = 0;
    let rightSide = list.length - 1;

    while (leftSide <= rightSide) {
      let centerIndex = Math.floor((leftSide + rightSide) / 2);
  ...
  // Пошук
  const foundPosition = searchByLength(peopleNames, neededLength);
